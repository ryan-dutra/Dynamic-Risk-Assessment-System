...
def _ingestion_timing():
    ...
    _ = subprocess.run(['python3', 'ingestion.py'], capture_output=True)
...
def _training_timing():
    ...
    """
    starttime = timeit.default_timer()
    _ = subprocess.run(['python3', 'training.py'], capture_output=True)
    timing = timeit.default_timer() - starttime
    return timing
...
def outdated_packages_list():
    ...
    dep = result.stdout.split('\n')
    # Skip the header and any empty lines
    dep = [line.split() for line in dep[2:] if line]

    # Make sure each line has exactly 3 elements
    dep_list = [line for line in dep if len(line) == 3]
    return dep_list
